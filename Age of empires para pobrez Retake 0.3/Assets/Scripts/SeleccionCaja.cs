...
public class SeleccionCaja : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            {
                ControladorSelecUnidad.Instance.DeselecAll();
                SelectUnits();
            }
    ...
    void SelectUnits()
    {
        foreach (var unidad in ControladorSelecUnidad.Instance.TodasUnidads)
        {
            if (selectionBox.Contains(myCam.WorldToScreenPoint(unidad.transform.position)))
            {
                ControladorSelecUnidad.Instance.ArrastreSelect(unidad);
            }
        }
    }
}